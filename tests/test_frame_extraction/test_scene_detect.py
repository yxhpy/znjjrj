...
class TestSceneDetectExtractor(BaseFrameExtractorTest):
    """PySceneDetect关键帧提取器测试类"""
    ...
    def test_extractor_creation(self):
        """测试PySceneDetect提取器创建"""
        logger.info("开始测试PySceneDetect提取器创建")
        extractor = create_frame_extractor()
        self.assertIsInstance(extractor, SceneDetectFrameExtractor)
        logger.info("成功创建PySceneDetect提取器")
    
    def test_frame_extraction(self):
        """测试帧提取功能"""
        logger.info("开始测试PySceneDetect帧提取功能")
